...
def train_detector(model, dataset, cfg, distributed=False, validate=False, logger=None,
                   use_seq_sampler=False):
    if logger is None:
        logger = get_root_logger(cfg.log_level)

    print(
        'in train_detector, distributed: {}'.format(distributed)
    )
    ...
    if use_seq_sampler:
        data_loaders = [
            build_dataloader(
                ...
                use_seq_sampler=True, max_epoches=cfg.total_epochs, max_training_seq=cfg.get('max_training_seq', None),
                training_seq_len=cfg.get('training_seq_len', None), repeat_test=cfg.get('training_seq_len', True)
            )
            for ds in dataset
        ]
    else:
