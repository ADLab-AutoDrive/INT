...
def collate_kitti(batch_list, samples_per_gpu=1):
    ...
    for key, elems in example_merged.items():
        if key in ["voxels", "num_points", "num_gt", "voxel_labels", "num_voxels",
                   "cyv_voxels", "cyv_num_points", "cyv_num_voxels", "points_seg_label"]:
            ...
            ret[key] = torch.tensor(np.concatenate(coors, axis=0))
                     ...
                     "ind", "mask", "cat", 'transform_matrix_list']:
        ...
        elif key in ['voxels_list', 'num_points_list', 'num_voxels_list', 'coords_list']:
            ...
            for elem in elems:
                for idx, ele in enumerate(elem):
                    ret[key][str(idx)].append(torch.tensor(ele))
            for kk, vv in ret[key].items():
                if key in ['coords_list']:
                    coors = []
                    for i, coor in enumerate(vv):
                        coor_pad = np.pad(coor, ((0, 0), (1, 0)), mode="constant", constant_values=i)
                        coors.append(coor_pad)
                    vv = coors

                res.append(torch.tensor(np.concatenate(vv, axis=0)))
            ret[key] = res

        elif key in ['gt_boxes_and_cls', 'transform_matrix']:
            if elems[0] is None:
                continue
