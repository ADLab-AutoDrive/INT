...
def save_checkpoint(model, filename, optimizer=None, meta=None, exclude_keys=None):
    ...
    model_state_dict = weights_to_cpu(model.state_dict())
    if exclude_keys is not None:
        assert isinstance(exclude_keys, list), type(exclude_keys)
        for key in exclude_keys:
            if key in model_state_dict:
                model_state_dict.pop(key)
            if f'single_det.{key}' in model_state_dict:
                model_state_dict.pop(f'single_det.{key}')

    checkpoint = {"meta": meta, "state_dict": model_state_dict}
