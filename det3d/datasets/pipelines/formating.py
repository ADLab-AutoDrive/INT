...
class Reformat(object):
    ...
    def __call__(self, res, info):
        ...
        if 'voxels' in res["lidar"]:
            ...
            if 'voxels_list' in voxels:
                assert res["mode"] in ["train", "val"], res["mode"]

                data_bundle.update(
                    voxels_list=voxels["voxels_list"],
                    shape=voxels["shape"],
                    num_points_list=voxels["num_points_list"],
                    num_voxels_list=voxels["num_voxels_list"],
                    coords_list=voxels["coords_list"],
        ...
        if 'transform_matrix' in res['lidar']:
            data_bundle.update(
                transform_matrix=res['lidar']['transform_matrix'],
            )
