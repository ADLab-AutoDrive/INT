...
class OpenPCDetWaymoDetectionMetricsEstimator(tf.test.TestCase):
    ...
    def generate_waymo_type_results(self, infos, class_names, is_gt=False):
        ...
        for frame_index, info in enumerate(infos):
            if is_gt:
                ...
                boxes3d.append(info['gt_box3d_lidar'][box_mask])
            else:
                num_boxes = len(info['box3d_lidar'])
                difficulty.append([0] * num_boxes)
                score.append(info['scores'])
                boxes3d.append(np.array(info['box3d_lidar']))
                box_name = info['name']
    ...
    def waymo_evaluation(self, prediction_infos, gt_infos, class_name, distance_thresh=100):
        ...
        gt_frameid, gt_boxes3d, gt_type, gt_score, gt_overlap_nlz, gt_difficulty = self.generate_waymo_type_results(
            gt_infos, class_name, is_gt=True
        )
