...
def _fill_infos(root_path, frames, split='train', nsweeps=1):
    ...
    for frame_name in tqdm(frames):  # global id
        ...
        if split != 'test':
            ...
            # add difficulty & num_points_in_gt & track_id
            difficulty = np.array([ann['detection_difficulty_level'] for ann in annos])[mask_not_zero]
            info['difficulty'] = difficulty.astype(np.int32)
            info['num_points_in_gt'] = num_points_in_gt[mask_not_zero].astype(np.int32)
            obj_ids = np.array([ann['id'] for ann in annos])[mask_not_zero]
            info['obj_ids'] = obj_ids.astype(np.int32)

        infos.append(info)
    return infos


def _fill_infos_seq_wise(root_path, frames, split='train'):
    # load all train infos
    infos = defaultdict(list)
    for frame_name in tqdm(frames):  # global id
        seq_name = '_'.join(frame_name.split('_')[0:2])
        frame_id = int(frame_name.split("_")[3][:-4])  # remove .pkl
        ...
        info = {
            ...
            "token": frame_name,
            "timestamp": ref_time,
            "global_from_car": ref_pose,
            "seq_tag": f"{seq_name}::{frame_id}::1",
        }
        ...
        if split != 'test':
            ...
            # add difficulty & num_points_in_gt & obj_name
            difficulty = np.array([ann['detection_difficulty_level'] for ann in annos])[mask_not_zero]
            info['difficulty'] = difficulty.astype(np.int32)
            info['num_points_in_gt'] = num_points_in_gt[mask_not_zero].astype(np.int32)
            obj_names = np.array([ann['name'] for ann in annos])[mask_not_zero]
            info['obj_names'] = obj_names.astype(str)

        infos[seq_name].append(info)

    ## check
    for seq_name, annos in infos.items():
        timestamps = np.array([ann['timestamp'] for ann in annos])
        ts_diff = np.diff(timestamps)
        assert np.all(ts_diff >= 0), seq_name
...
def create_waymo_infos(root_path, split='train', nsweeps=1, output_dir=None,
                       make_seq_wise_info=False):
    frames = get_available_frames(root_path, split)

    if not make_seq_wise_info:
        waymo_infos = _fill_infos(root_path, frames, split, nsweeps)
    else:
        waymo_infos = _fill_infos_seq_wise(root_path, frames, split)
    ...
    if not make_seq_wise_info:
        out_path = os.path.join(output_dir, "infos_" + split + "_{:02d}sweeps_filter_zero_gt.pkl".format(nsweeps))
    else:
        out_path = os.path.join(output_dir, "infos_" + split + "_seq_filter_zero_gt.pkl".format(nsweeps))
