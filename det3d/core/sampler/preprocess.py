...
class BatchSamplerSeq:
    def __init__(self, sampled_list, max_num, name=None):
        self._sampled_list = sampled_list
        self._seed = None
        self._example_num = len(sampled_list)
        self._name = name
        ...
        assert self._max_num <= self._example_num

    def set_seed(self, seed):
        self._seed = seed

    def sample(self, num, train_seq_len_and_cur_frame):
        '''
        train_seq_len_and_cur_frame: [train_len, cur_frame], eg: [10, 3]
        '''
        train_seq_len, cur_frame = train_seq_len_and_cur_frame
        np.random.seed(self._seed)
        selected_indices = np.random.choice(self._example_num, self._max_num, replace=False)
        start_pos = np.random.rand(self._max_num)

        used_indices = selected_indices[:num]
        used_start_pos = start_pos[:num]

        ret_samples = []
        for idx, s_pos in zip(used_indices, used_start_pos):
            obj_seq = self._sampled_list[idx]
            s_pos_int = int(len(obj_seq) * s_pos)
            if (s_pos_int + train_seq_len) > len(obj_seq):
                s_pos_int = max(len(obj_seq) - train_seq_len, 0)
            cur_sample_idx = min(s_pos_int + cur_frame, len(obj_seq) - 1)
            ret_samples.append(obj_seq[cur_sample_idx])
...
def global_rotation(gt_boxes, points, rotation=np.pi / 4, random_state=None,
                    return_random_state=False, return_rotation_ang=False):
...
def random_flip_both(gt_boxes, points, probability=0.5, flip_coor=None, random_state=None,
                     return_random_state=False, return_flip_state=False):
    flip_x, flip_y = False, False
    if random_state is not None:
        st0, st1 = random_state
    else:
        st0, st1 = None, None
    # x flip
    if st0 is not None:
    ...
    if enable:
        flip_x = True
        gt_boxes[:, 1] = -gt_boxes[:, 1]
        gt_boxes[:, -1] = -gt_boxes[:, -1] + np.pi  # TODO seems wrong, -gt_boxes[:, -1]
        points[:, 1] = -points[:, 1]
        if gt_boxes.shape[1] > 7:  # y axis: x, y, z, w, h, l, vx, vy, r
            gt_boxes[:, 7] = -gt_boxes[:, 7]
    ...
    if enable:
        ...
        gt_boxes[:, -1] = -gt_boxes[:, -1] + 2 * np.pi  # TODO: CHECK THIS seems wrong, -gt_boxes[:, -1]+np.pi
    ...
    if return_random_state:
        ret += [[st0, st1]]
    if return_flip_state:
        ret += [[flip_x, flip_y]]
    return ret


def global_scaling_v2(gt_boxes, points, min_scale=0.95, max_scale=1.05,
                      random_state=None, return_random_state=False, return_noise_scale=False):
    if random_state is not None:
        st0 = random_state[0]
        np.random.set_state(st0)
    else:
...
def global_translate_(gt_boxes, points, noise_translate_std,
                      random_state=None, return_random_state=False, return_noise_trans=False):
    """
    Apply global translation to gt_boxes and points.
    """
    if random_state is not None:
        st0, st1, st2 = random_state
    else:
        st0, st1, st2 = None, None, None
    ...
    if st0 is not None:
        ...
        st0 = np.random.get_state()
    n0 = np.random.normal(0, noise_translate_std[0], 1)

    if st1 is not None:
        np.random.set_state(st1)
    else:
        st1 = np.random.get_state()
    n1 = np.random.normal(0, noise_translate_std[0], 1)

    if st2 is not None:
        np.random.set_state(st2)
    else:
        st2 = np.random.get_state()
    n2 = np.random.normal(0, noise_translate_std[0], 1)

    noise_translate = np.array([n0, n1, n2]).T
