...
class DataBaseSamplerV2:
    def __init__(
        ...
        for k, v in self._group_db_infos.items():
            if self.seq_gt_db:
                max_sample_num = self._sample_max_nums[self._sample_classes.index(k)]
                self._sampler_dict[k] = prep.BatchSamplerSeq(v, max_sample_num, k)
            else:
    ...
    def sample_all(
        ...
        if self.seq_gt_db:
            # fix random seed
            for name in self._sampler_dict:
                self._sampler_dict[name].set_seed(np.random.randint(1e6))
            ...
            if sampled_num > 0:
                if self._use_group_sampling:
                    ...
                    sampled_cls = self.sample_class_v2(
                        class_name, sampled_num, avoid_coll_boxes, train_seq_len_and_cur_frame
                    )
    ...
    def sample_class_v2(self, name, num, gt_boxes, train_seq_len_and_cur_frame=None):
        if self.seq_gt_db:
            sampled = self._sampler_dict[name].sample(num, train_seq_len_and_cur_frame)
        else:
