...
def get_downsample_factor_swin(model_config):
    try:
        neck_cfg = model_config["neck"]
    except:
        model_config = model_config['first_stage_cfg']
    ...
    swin_cfgs = neck_cfg['swin_cfgs']
    fpn_cfgs = neck_cfg['fpn_cfgs']

    downsample_factor = swin_cfgs['patch_size'] * 2 ** (len(swin_cfgs["depths"]) - 1)
    if len(fpn_cfgs.get("us_layer_strides", [])) > 0:
        downsample_factor /= fpn_cfgs.get("us_layer_strides", [])[-1]
